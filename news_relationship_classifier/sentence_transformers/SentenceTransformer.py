...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
            ...
            output_file: str,   # information during training
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if training_steps % 200 == 0:
                    print({'epoch': epoch, 'global_step': global_step,
                           'training_steps': training_steps, 'loss': float(loss_value)})
                    training_inf.append({'epoch': epoch, 'global_step': global_step,
                                         'training_steps': training_steps, 'loss': float(loss_value)})
                    json.dump(training_inf, open(output_file, 'w'), indent=4)
